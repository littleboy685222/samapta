...
if ($result_check->num_rows > 0) {
    if ($reset) {
        // Reset sit_up_count to 0
        $sql_update = "UPDATE data_kadet SET sit_up_count = 0 WHERE UID = ?";
    } else {
        // Increment sit_up_count by 1
        $sql_update = "UPDATE data_kadet SET sit_up_count = sit_up_count + 1 WHERE UID = ?";
    }
    ...
    if ($stmt_update->execute()) {
        ...
        $sql_get = "SELECT nama, nim, prodi, sit_up_count FROM data_kadet WHERE UID = ?";
