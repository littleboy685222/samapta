...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['uid'])) {
    ...
    if (trim($status) === "mulai") {
        // Memeriksa apakah UID sudah ada dalam tabel jumlah_tap
        $query_check = "SELECT jumlah_tap FROM shuttlerun_status WHERE uid = ?";
        ...
        if ($result_check->num_rows > 0) {
            // Jika UID sudah ada, update jumlah_tap
            $query_update = "UPDATE shuttlerun_status SET jumlah_tap = jumlah_tap + 1 WHERE uid = ?";
            ...
            // Jika UID belum ada, tambahkan sebagai baru dengan jumlah_tap awal 1
            $query_insert = "INSERT INTO shuttlerun_status (uid, jumlah_tap) VALUES (?, 1)";
