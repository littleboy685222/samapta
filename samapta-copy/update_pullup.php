...
if ($result_check->num_rows > 0) {
    if ($reset) {
        // Reset pull_up_count to 0
        $sql_update = "UPDATE data_kadet SET pull_up_count = 0 WHERE UID = ?";
    } else {
        // Increment pull_up_count by 1
        $sql_update = "UPDATE data_kadet SET pull_up_count = pull_up_count + 1 WHERE UID = ?";
    }
    ...
    if ($stmt_update->execute()) {
        ...
        $sql_get = "SELECT nama, nim, prodi, pull_up_count FROM data_kadet WHERE UID = ?";
        ...
        echo json_encode([
            ...
            "nim" => $row['nim'],
            "prodi" => $row['prodi'],
            "pull_up_count" => $row['pull_up_count']
        ]);
    } else {
